	...
	public partial class ComicBookDialog : Form
	{
		...
		private void SetComicToEditor(ComicBook comic)
		{
			...
			new Control[4]
			{
				...
				labelPages,
				lblPages
			}.ForEach(delegate(Control c)
			{
				c.Visible = comic.IsLinked;
			});
			SplitButton splitButton = btThumbnail;
			bool flag2 = (btLinkFile.Visible = !comic.IsLinked && comic.EditMode.IsLocalComic());
			bool allowDrop = (splitButton.Visible = flag2);
			AllowDrop = allowDrop;
			Label label = labelWhere;
			Panel panel = whereSeparator;
			flag2 = (lblPath.Visible = comic.IsLinked && comic.EditMode.IsLocalComic());
			allowDrop = (panel.Visible = flag2);
			label.Visible = allowDrop;
			...
			book?.Dispose();
			bool flag6 = !comic.IsLinked || comic.IsInContainer;
			bool flag7 = flag6 || Program.Settings.UpdateComicFiles;
			bool flag8 = comic.EditMode.CanEditProperties();
			TabPage tabPage = tabDetails;
			TabPage tabPage2 = tabPlot;
			flag2 = (tabColors.Enabled = flag7 && flag8);
			allowDrop = (tabPage2.Enabled = flag2);
			tabPage.Enabled = allowDrop;
			tabPages.Enabled = flag7 && comic.EditMode.CanEditPages();
			TabPage tabPage3 = tabCatalog;
			allowDrop = (tabCustom.Enabled = flag6 && flag8);
			tabPage3.Enabled = allowDrop;
			EnableTabPage(tabPages, comic.IsLinked);
			EnableTabPage(tabColors, comic.IsLinked);
			EnableTabPage(tabCatalog, (!comic.IsLinked || !Program.Settings.CatalogOnlyForFileless) && flag6);
			EnableTabPage(tabCustom, Program.Settings.ShowCustomBookFields && flag6);
			Label label2 = labelEnableProposed;
			ComboBox comboBox = cbEnableProposed;
			Label label3 = labelScanInformation;
			bool flag12 = (txScanInformation.Visible = comic.IsLinked);
			flag2 = (label3.Visible = flag12);
			allowDrop = (comboBox.Visible = flag2);
			label2.Visible = allowDrop;
			Label label4 = labelOpenedTime;
			NullableDateTimePicker nullableDateTimePicker = dtpOpenedTime;
			Label label5 = labelPagesAsTextSimple;
			flag12 = (txPagesAsTextSimple.Visible = !comic.IsLinked);
			flag2 = (label5.Visible = flag12);
			allowDrop = (nullableDateTimePicker.Visible = flag2);
			label4.Visible = allowDrop;
			RatingControl ratingControl = txCommunityRating;
			RatingControl ratingControl2 = txRating;
			TextBoxEx textBoxEx = txTags;
			ComboBox comboBox2 = cbEnableProposed;
			bool flag19 = (cbSeriesComplete.Enabled = flag6);
			flag12 = (comboBox2.Enabled = flag19);
			flag2 = (textBoxEx.Enabled = flag12);
			allowDrop = (ratingControl2.Enabled = flag2);
			ratingControl.Enabled = allowDrop;
			if (!flag8)
			{
				RatingControl ratingControl3 = txCommunityRating;
				allowDrop = (txRating.Enabled = false);
				ratingControl3.Enabled = allowDrop;
			}
