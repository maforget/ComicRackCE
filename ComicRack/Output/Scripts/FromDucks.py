...
def ReadInfoDucks(book):
	...
	#	pass
	#
	if DEBUG:log_BD("Web ----------> " + cWeb + cSeries)
	...
	#
	if DEBUG:log_BD("Language -----> " + Language)
	#
	...
	#
	if DEBUG:log_BD("Publisher ----> " + Publisher)
	#
	...
	#
	if DEBUG:log_BD("Main Title ---> " + MainTitle)
	#
	...
	#
	if DEBUG:log_BD("Day   -------->" , Day)
	if DEBUG:log_BD("Month -------->" , Month)
	if DEBUG:log_BD("Year  -------->" , Year)
	#
	...
	nl=0
	if DEBUG:log_BD(chr(10) + "<-------- Stories -------->")
	...
	for Stories in result3:
		...
		#
		if DEBUG:log_BD(chr(10) + "Type ---------> " + TypeCol(Type))
		#
		if DEBUG:log_BD("Story # ------> " + Story)
		#
		if DEBUG:log_BD("Title --------> " + Title)
		#
		...
		#
		if DEBUG:log_BD("Artists ------> ")
		for all in Artists:
			if DEBUG:log_BD(" " * 16 + all.replace("^",": "))
		#
		...
		#
		if DEBUG:log_BD("Characters ---> ")
		for all in Appearances:
			try:
				if DEBUG:log_BD(" " * 16 + all[1].decode('utf-8'))
			except:
				if DEBUG:log_BD(" " * 16 + all[1])
		#
