    ...
    public abstract class PackedStorageProvider : StorageProvider
    {
		...
		protected override ComicInfo OnStore(IImageProvider provider, ComicInfo info, string target, StorageSetting setting)
        {
            ...
            try
            {
                ...
                Parallel.For(0, provider.Count, parallelOptions, (int n, ParallelLoopState ls) =>
                {
                    try
                    {
                ...
                // If the original info is a ComicBook we need to keep it's type for AddBook, otherwise we will lose the properties that are only in ComicBook
                ComicInfo comicInfo = info is ComicBook cb ? cb.Clone<ComicBook>() : new ComicInfo(info);
                ...
                if (setting.EmbedComicInfo)
                    AddInfo(comicInfo.GetInfo()); // Force the type to ComicInfo otherwise so it doesn't output a ComicBook

                if (setting.EmbedComicInfo && setting.EmbedComicBook && comicInfo is ComicBook comicBook)
                    AddBook(comicBook);
