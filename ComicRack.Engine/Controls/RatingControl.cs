    ...
    public class RatingControl : Control
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (drawBorder)
            {
                if (ThemeExtensions.IsDarkModeEnabled)
                {
                    ControlPaint.DrawBorder(graphics, base.ClientRectangle, ThemeExtensions.Colors.DefaultBorder, ButtonBorderStyle.Inset);
                }
                else
                {
                    if (Focused)
        ...
        private void DrawContent(Graphics gr)
        {
            ...
            if (drawText)
            {
                ...
                if (ThemeExtensions.IsDarkModeEnabled)
                    color = ((Rating == 0f) ? SystemColors.GrayText : SystemColors.ControlText);
