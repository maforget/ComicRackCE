	...
	public class SplitButton : Button
	{
		...
		private void DrawButtonText(Graphics graphics, Rectangle rectangle)
		{
			...
			if (!wasDrawn)
			{
				if (base.Enabled)
					...
					ControlPaint.DrawStringDisabled(graphics, Text, Font, SystemColors.GrayText, rectangle, textFormatFlags);
			}
		}
		...
		protected override void OnPaint(PaintEventArgs pevent)
		{
			...
			if (State != PushButtonState.Pressed && Focused)
				ThemeExtensions.TryDrawTheme(() => ControlPaint.DrawFocusRectangle(graphics, rectangle), onlyDrawIfDefault: true);
		}
