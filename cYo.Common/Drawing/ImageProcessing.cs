    ...
    public static class ImageProcessing
    {
        ...
        public unsafe static void Invert(this Bitmap bitmap, Rectangle clipRectangle)
        {
            ...
            try
            {
                ...
                Parallel.For(0, clipRectangle.Height, (int h) =>
                {
        ...
        public unsafe static void Convolute(this Bitmap bitmap, Rectangle clipRectangle, ConvolutionMatrix m)
        {
            ...
            using (Bitmap bitmap2 = bitmap.Clone(clipRectangle, bitmap.PixelFormat))
            {
                ...
                try
                {
                    ...
                    Parallel.For(0, toExclusive, (int h) =>
                    {
        ...
        public unsafe static Bitmap ResizeFast(Bitmap source, int newWidth, int newHeight, PixelFormat format, ResizeFastInterpolation method)
        {
            ...
            try
            {
                ...
                int srcPixelSize = Image.GetPixelFormatSize(bitmap.PixelFormat) / 8;
                //int srcOffset = srcStride - srcPixelSize * width;

                int dstStride = dstData.Stride;
                int dstPixelSize = Image.GetPixelFormatSize(format) / 8;
                    ...
                    case ResizeFastInterpolation.NearestNeighbor:
                        {
                            ...
                            Parallel.For(0, newHeight, (int y) =>
                            {
                                ...
                                // Use long to prevent overflow for high-res images
                                byte* rowDst = orgdst + (long)dstStride * y;
                                byte* rowSrc = orgsrc + (long)srcStride * Math.Min((int)(y * yFactor), height - 1); // Clamp the y index incase of rounding errors it exceeds the source image height

                                // for each pixel
                                for (int x = 0; x < newWidth; x++)
                                {
                                    byte* p = rowSrc + (srcPixelSize * Math.Min((int)(x * xFactor), width - 1)); // Clamp the x index incase of rounding errors it exceeds the source image width
                                    byte* dst = rowDst + (x * dstPixelSize); // Calculate pixel destination pointer directly
                    ...
                    case ResizeFastInterpolation.Bilinear:
                        {
                            ...
                            Parallel.For(0, newHeight, (int y) =>
                            {
                                byte* rowDst = orgdst + (long)y * dstStride; // Use long to prevent overflow for high-res images

                                // Y coordinates
                                float oy = (float)y * yFactor;
                                int oy1 = (int)oy;
                                int oy2 = Math.Min(oy1 + 1, ymax);
                                float dy1 = oy - (float)oy1;
                                float dy2 = 1f - dy1;

                                // get temp pointers
                                byte* tp1 = orgsrc + ((long)oy1 * srcStride); // Use long to prevent overflow for high-res images
                                byte* tp2 = orgsrc + ((long)oy2 * srcStride); // Use long to prevent overflow for high-res images
                                ...
                                for (int x = 0; x < newWidth; x++)
                                {
                                    ...
                                    int ox1 = (int)ox;
                                    int ox2 = Math.Min(ox1 + 1, xmax);
                                    float dx1 = ox - (float)ox1;
                                    float dx2 = 1f - dx1;

                                    // get four points
                                    byte* p1 = tp1 + (long)ox1 * srcPixelSize;
                                    byte* p2 = tp1 + (long)ox2 * srcPixelSize;
                                    byte* p3 = tp2 + (long)ox1 * srcPixelSize;
                                    byte* p4 = tp2 + (long)ox2 * srcPixelSize;

                                    byte* dst = rowDst + (long)x * dstPixelSize; // Calculate pixel destination pointer directly

                                    for (int i = 0; i < minPixelSize; i++)
                                    {
                                        dst[i] = (byte)(
                                            dy2 * (dx2 * p1[i] + dx1 * p2[i]) +
                                            dy1 * (dx2 * p3[i] + dx1 * p4[i]));
                                    }
                                }
                            });
                        }
                    ...
                    case ResizeFastInterpolation.Bicubic:
                        {
                            ...
                            Parallel.For(0, newHeight, (int y) =>
                            {
                                byte* rowDst = orgdst + (long)y * dstStride;
                                ...
                                // Use stackalloc to avoid heap allocation in the loop!
                                float* accumulator = stackalloc float[minPixelSize];
                                ...
                                for (int x = 0; x < newWidth; x++)
                                {
                                    ...
                                    for (int n = -1; n < 3; n++)
                                    {
                                        ...
                                        int oy2 = Math.Max(0, Math.Min(oy1 + n, ymax));

                                        byte* rowSrc = orgsrc + (long)oy2 * srcStride; // Calculate pixel destination pointer directly

                                        for (int m = -1; m < 3; m++)
                                        {
                                            // get X coefficient
                                            float k2 = k1 * BiCubicKernel((float)m - dx);
                                            int ox2 = Math.Max(0, Math.Min(ox1 + m, xmax));

                                            // temporary pointer
                                            byte* p = rowSrc + (long)ox2 * srcPixelSize;

                                            for (int i = 0; i < minPixelSize; i++)
                                            {
                                                accumulator[i] += k2 * p[i];
                                            }
                                        }
                                    }

                                    byte* dst = rowDst + (long)x * dstPixelSize;

                                    for (int i = 0; i < minPixelSize; i++)
                                    {
                                        dst[i] = (byte)Math.Max(0, Math.Min(255, (int)accumulator[i]));
                                    }
                                }
                            });
                        }
        ...
        {
            processingStack.ForEach((ImageAction a) => a(bitmap));
        }
    }
}
